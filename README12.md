...
# 10-1 何分前にメッセージが送信されたか表示する

## 送信日時の表示方法を変更する

+ `root docker compose run --rm front npm install date-fns --save`を実行<br>
...
<template>
  <div class="container">
    <navbar-menu />
    <chat-window @connectCable="connectCable" :messages="formattedMessages" /> // 編集
...
import { formatDistanceToNow } from "date-fns"; // 追加
    ...
    <div v-if="messages" class="messages">
      <ul v-for="message in messages" :key="message.id">
        ...
        >
          ...
          <span class="created-at">{{ message.created_at }}前</span> // 編集
        </li>
      </ul>
    </div>
  </div>
